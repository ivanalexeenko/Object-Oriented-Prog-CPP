...
ostream & operator<<(ostream & os, CountSum & CS)
{
	return os << CS.sum;
}

ostream & operator<<(ostream & os, ConcatKeys & CK)
{
	return os << CK.str;
}
...
void kHashTable::accept(Visitor & v)
{
	v.visit(this);
}

PAIR * kHashTable::Begin() const
{
	return arr;
}

PAIR * kHashTable::End() const
{
	return arr+size;
}
...
void kHashTable::Swap(kHashTable & kHT)
{
	kHashTable temp;
	temp = *this;
	*this = kHT;
...
void CountSum::visit(kHashTable * kHT)
{
	sum = 0;
	for (HashIterator i(*kHT); !i.IsDone(); i.Next())
	{
		sum += i->second;
	}
}
...
void ConcatKeys::visit(kHashTable * kHT)
{
	str = "";
	for (HashIterator i(*kHT); !i.IsDone(); i.Next())
	{
